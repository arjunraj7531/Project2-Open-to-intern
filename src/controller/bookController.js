...
const getBooks = async function(req, res) {
    ...
    try {
        ...
        for (let key in query) {
            if (key != "userId" && key !== "category" && key !== "subcategory") return res.status(400).send({ status: false, message: `Invalid key:-${key} in query params only userId , category , subcategory is allowed` })
        }

        let findObj = { isDeleted: false, ...query }

        //  Below line for trim() and validate userId value. 
        ...
        if (allBook.length <= 0)
            return res.status(404).send({ status: false, message: "No book found (With given query)" })
...
const updateBookById = async function(req, res) {
    try {
        ...
        const updatedBook = await bookModel.findByIdAndUpdate({ _id: bookId }, { $set: { title: title, excerpt: excerpt, releasedAt: releasedAt, ISBN: ISBN, releasedAt: releasedAt } }, { new: true })
